...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    ...
    GLint lightDirIndex = glGetUniformLocation(_esContext.program, "lightDir");
    GLint viewPosIndex = glGetUniformLocation(_esContext.program, "viewPos");
    
    GLKMatrix4 rotate = GLKMatrix4MakeRotation(0, 0,1,0);
    ...
    //设置光源位置（世界空间坐标系）
    static float lightDir[3] = {10.0,20.0,15.f};
    
    //这里设置观察点为摄像机的位置(cameraMatrix里eye的位置)
    static float viewPos[3] = {0.0,-0.5,2.0};
    
    glUniform3fv(lightDirIndex, 1, lightDir);
...
//绘制立方体
- (void)drawCubeWithTextCoordLocation:(GLuint)textCoordIndex andPositionlocation:(GLuint)positionIndex andNormalLocation:(GLuint)normalIndex {
